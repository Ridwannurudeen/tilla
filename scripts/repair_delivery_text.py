"""Repair the stores that still promise a download at a URL that 404s.
...
after settling. The default was fixed on 2026-07-28, which changed what NEW stores
get and left every row already persisted exactly as it was.

This rewrites those rows to the honest text (`engine.default_delivery_text`, the
same function the create path calls, so the two cannot drift) and repairs the
on-disk `stores/<slug>/store.json` to match, since that file is what
`app.import_stores` would re-import on a fresh host.

Deliberately narrow — a store is rewritten only when BOTH hold:

  * its `delivery` still contains the fabricated marker, and
...
price floor that started all of this. A merchant who wrote their own delivery text
is untouched too, because theirs does not carry the marker.
