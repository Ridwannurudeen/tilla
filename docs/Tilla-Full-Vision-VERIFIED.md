...
- **Agents hiring Tilla** — proven with an on-chain receipt (a self-funded arm's-length wallet paid Tilla to create a store, block 65875359) [live/proven]. **Tilla hiring Warden** (paid content-safety screening) [**live/proven** as of 2026-07-25 — updated 2026-07-26; this line previously read "the paid hire is still OFF … no agent-to-agent screening fee has ever been paid", which was true only of the earlier demo-mode screenings. A paid hire has since settled: 0.1 USDT0, block 66208040, and the verdict gated real content — PROOF §11]. Task-board participation [designed].
...
- **Proven since (2026‑07‑24), each a self-funded arm's-length test unless noted:** a **subscription** rail settling twice (blocks 66072022, 66072295); an **`aggr_deferred` batch settle** covering two orders (block 66059520); a **full escrow fund→release cycle** (blocks 66036695 / 66036700); a **completed ACP checkout session** (block 66025300); **4 EAS attestation receipts** written on-chain; and **a merchant paying the 0.05 USDT0 fee from the dashboard** and getting the live store `/s/jersey-fc/` back (block 66151616) — the human self-serve path proven end-to-end with real money. Eighteen stores now exist (count re-read from the production DB 2026-07-26; this said seventeen), and discovery ranks them on earned outcomes (`trust_tier`, `success_rate`, `review_avg`).
- **Still unproven, and not claimed** *(updated 2026-07-26 — two of the three items here have since been proven; kept visible rather than deleted)*: ~~the MPP channel has been opened with a signed voucher (2 USDT deposit) but **never closed or settled**~~ → **both channels closed on-chain 2026-07-23** (blocks 66056578 / 66056643; 0.1 to the merchant, 3.9 refunded — PROOF §10). ~~the paid Warden hire is off (all 13 screenings ran in free demo mode)~~ → **a paid hire settled 2026-07-25** (0.1 USDT0, block 66208040 — PROOF §11). What genuinely remains unproven: **none of this is external customer demand** — every settlement is a self-funded arm's-length test.
...
12. ~~The **paid Warden hire stays OFF** — all 13 screenings ran in free demo mode; no agent-to-agent screening fee has ever been paid.~~ **Superseded 2026-07-26:** a paid hire settled 2026-07-25 (0.1 USDT0, block 66208040) and the verdict gated real content — PROOF §11. Note the receipt lives in `event_log` as `hire.rating`, not as a `mode='paid'` row in `screening_receipts` (all 14 rows there remain `demo`).
