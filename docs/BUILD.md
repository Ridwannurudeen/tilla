...
### M8 — Payment methods expansion (spike-gated, ROADMAP §9)
(a) `aggr_deferred` second accepts-entry on batch-priced SKUs (server class installed; treat `pending` settle as success-settling; TEE-buyer caveat documented). (b) MPP pay-as-you-go: after spike passes — channel open/voucher/topup/close endpoints for metered SKUs, `FileStore`→sqlite channel state. (c) Subscriptions: Node sidecar spike; if green, sidecar service + FastAPI proxy for `period` SKUs.
**Accept per rail (binary):** the rail is LIVE with a real settlement tx hash logged — otherwise the module's scope formally shrinks, ROADMAP Phase 4 is marked partial, and no claim about that rail appears anywhere. A parked rail is a scope cut, not a pass.
