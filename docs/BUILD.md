...
**Current live state (do not break):** https://tilla.gudman.xyz (nginx + `tilla-api.service`, uvicorn 127.0.0.1:8040), example store `/s/invoice-flow/`, ASP #6961 x402-gated `/create-store` (0.05 USDT0 — `PAYMENT_AMOUNT = "50000"` at 6dp; the fee was 1 USDT when this line was written). `/opt/tilla` = 4 files, 560 lines, **no git, no tests, in-memory checkouts, unescaped LLM HTML (live XSS), fake delivery string**.
...
### M8 — Payment methods expansion (built; all four rails now enabled in prod — see the per-rail status below)
...
The three flags above (`TILLA_AGGR_DEFERRED`, `TILLA_MPP_ENABLED`, `TILLA_SUBSCRIPTIONS_ENABLED`) still default OFF **in code** — they are set in the production environment, which is what "enabled" below means.

**Accept per rail (binary):** a rail is CLAIMED working ONLY if a real settlement tx hash is logged; otherwise it is honestly PARKED (scope cut) and no claim about it appears anywhere. Never fake/simulate a settlement.

**Per-rail status (re-verified against production 2026-07-24 — every settlement below is a self-funded arm's-length test):**
1. **x402 exact — LIVE, PROVEN** (3 recorded settlement proofs; unchanged).
2. **aggr_deferred — LIVE, PROVEN.** Enabled, and one real **batch settle covering two orders** is logged (block 66059520). Standing caveats, unchanged: a settle requires an OKX **TEE agentic-wallet buyer** (plain EOAs are rejected at verify), and a failed EOA attempt is recorded as **buyer-class-gated**, never as a rail result. Note for anyone reading a live 402: **no batch-priced product is currently listed**, so today's challenges carry only the `exact` accepts-entry.
3. **MPP pay-as-you-go — LIVE, FULLY PROVEN.** Enabled, and the whole loop has settled on-chain: a channel **opened with a signed voucher** against a 2 USDT deposit, a metered unit delivered, then the channel **closed on-chain** (block 66056578) paying 0.1 USDT0 to the merchant and refunding 1.9 to the payer. A second raw test channel closed the same way with a full 2 USDT refund. Receipts and the escrow-balance reconciliation are in `docs/PROOF-onchain.md` §10. Note that OKX's settlement-agent API still reports both channels as `CLOSING` — that bookkeeping is stale; the chain is the authority.
4. **subscription (period) — LIVE, PROVEN.** Sidecar + proxy enabled, with **two real charges settled on-chain** (blocks 66072022, 66072295).
...
**Committed (x402/HTTP only — no XMTP dependency):** additional services under #6961 (`upgrade-store`, `add-product`); store-as-ASP Option A (delta-service per store under one ASP — proven pattern from Warden 35484); agents-hiring-agents: Tilla pays Warden #3808 via x402 for each screening (plain HTTP 402 flow; real receipt, shown in dashboard) — **PROVEN as of 2026-07-25** *(updated 2026-07-26; this line previously read "still OFF as of 2026-07-24 … no screening fee has ever been paid and the hire is not claimed anywhere", which described only the earlier demo-mode screenings)*: a paid hire settled 0.1 USDT0 at block 66208040 and the verdict gated real content — PROOF §11, and it is now claimed in the submission materials. Option B (per-store ASP identity) as measured experiment (1 store, review-throughput logged).
...
**STATUS 2026-07-24: the attester is ENABLED in production and 4 real attestations are recorded on-chain.** The gates and defaults described below are unchanged in code — they are simply satisfied in the production environment now. ~~`StoreRegistry.sol` is still PREPARED, NOT DEPLOYED.~~ **Corrected 2026-07-26: it was already deployed when that line was written.** `StoreRegistry.sol` is live on X Layer mainnet at `0x4507701110396B8B4204698ABf760Dd5418BfCe6` (deployed 2026-07-23, bytecode confirmed on-chain, 5 stores registered, Sourcify-verified) — see `contracts/README.md`.

**BUILD STATUS (branch `m11/onchain-depth`, as shipped):** code shipped DORMANT. `app/attest.py`
...
- **ACP** (`app/acp.py`): five `/s/{slug}/checkout_sessions` endpoints over the M3 order machinery, mounted behind `TILLA_ACP_ENABLED` — **enabled in production as of 2026-07-24** (the endpoints answer 401/422, not the dormant 503) with one completed session settled on-chain (block 66025300, a self-funded arm's-length test); `Idempotency-Key` per store, `API-Version` echoed, optional HMAC signature (`TILLA_ACP_SIGNING_SECRET`). Complete ships the **tx-hash / `onchain_usdt0`** mode (exact `verify_txhash`); the **x402-middleware complete mode is spike 9 — PARKED**, so tx-hash mode alone is claimed.
...
- **USER-gated / PARKED (never claimed until a real artifact exists):** affiliate payout execution (manual operator send); ChatGPT/Instant-Checkout + Perplexity + Google Merchant Center *listings* (external approval/enrollment); SMTP sends + broadcasts; ACP x402-complete (spike 9); affiliate-rate UI + non-address codes. (`TILLA_ACP_ENABLED` has since been flipped on, after a live tx-hash-complete session settled on-chain — see the M13 ACP line above.) See `docs/acp-checkout.md`.

### M14 — Phase 3–4 vision: the ONE buildable headline + growth-agent seed (post-hackathon)
The full Phase 3–4 "commerce OS" (plugin ecosystem, cross-chain, B2B/federation, autonomous growth agent) was, **at the time of this milestone**, multi-quarter vision — designed, not built. This milestone ships only the two slices that were buildable and honest then; no schema change (alembic head stays `0009`). *(Superseded since: those four pieces were subsequently built as M15–M18 — `docs/VISION.md` carries the SUPERSEDED banner and the per-section build map.)*
