...
**Committed (x402/HTTP only — no XMTP dependency):** additional services under #6961 (`upgrade-store`, `add-product`); store-as-ASP Option A (delta-service per store under one ASP — proven pattern from Warden 35484); agents-hiring-agents: Tilla pays Warden #3808 via x402 for each screening (plain HTTP 402 flow; real receipt, shown in dashboard) — **still OFF as of 2026-07-24: all 13 screenings to date ran in Warden's free demo mode, so no screening fee has ever been paid and the hire is not claimed anywhere**. Option B (per-store ASP identity) as measured experiment (1 store, review-throughput logged).
...
**STATUS 2026-07-24: the attester is ENABLED in production and 4 real attestations are recorded on-chain.** The gates and defaults described below are unchanged in code — they are simply satisfied in the production environment now. `StoreRegistry.sol` is still PREPARED, NOT DEPLOYED.
